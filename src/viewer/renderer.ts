...
import {
  ...
  selectedNode, focusNode, searchTerm, matchedNodes,
  hiddenKinds, cycleNodes, showCycles, layoutMode,
  directoryClusters, sim, simSettled, glRunning, setGlRunning,
  ViewNode, ViewEdge,
} from './state.js';
...
function renderCanvas2D() {
  ...
  ctx.beginPath();
  for (let x = Math.floor(minX / gridSize) * gridSize; x < maxX; x += gridSize) { ctx.moveTo(x, minY); ctx.lineTo(x, maxY); }
  for (let y = Math.floor(minY / gridSize) * gridSize; y < maxY; y += gridSize) { ctx.moveTo(minX, y); ctx.lineTo(maxX, y); }
  ctx.stroke();
  ...
  for (const dc of directoryClusters) {
    ...
    const cx2 = (dc.minX + dc.maxX) / 2, cy2 = dc.minY;
    ctx.fillStyle = '#161b22'; ctx.strokeStyle = '#21262d';
    ctx.lineWidth = 1 / transform.k;
    const rr = 8 / transform.k;
    ctx.beginPath();
    ctx.moveTo(dc.minX + rr, dc.minY); ctx.lineTo(dc.maxX - rr, dc.minY);
    ctx.quadraticCurveTo(dc.maxX, dc.minY, dc.maxX, dc.minY + rr);
    ctx.lineTo(dc.maxX, dc.maxY - rr); ctx.quadraticCurveTo(dc.maxX, dc.maxY, dc.maxX - rr, dc.maxY);
    ctx.lineTo(dc.minX + rr, dc.maxY); ctx.quadraticCurveTo(dc.minX, dc.maxY, dc.minX, dc.maxY - rr);
    ctx.lineTo(dc.minX, dc.minY + rr); ctx.quadraticCurveTo(dc.minX, dc.minY, dc.minX + rr, dc.minY);
  ...
  for (const e of edges) {
    ...
    let cycleEdge = false, related = false, isFocusRelated = false;
    ...
    if (showCycles && cycleNodes.size > 0 && cycleNodes.has(e.source.id) && cycleNodes.has(e.target.id)) cycleEdge = true;
  ...
  for (const n of drawnNodes) {
    ...
    const isHover = n === hoveredNode, isSel = n === selectedNode;
    ...
    if (isInCycle) { glowColor = '#f85149'; glowBlur = 16; }
    else if (isSel) { glowColor = '#58a6ff'; glowBlur = 20; }
    else if (isHover) { glowColor = color; glowBlur = 12; }
    ...
    const r = size;
    if (n.kind === 'file') { ctx.arc(0, 0, r, 0, Math.PI * 2); }
    else if (n.kind === 'function') { ctx.moveTo(0, -r); ctx.lineTo(r, 0); ctx.lineTo(0, r); ctx.lineTo(-r, 0); ctx.closePath(); }
    else if (n.kind === 'class') { const rr2 = Math.min(4 / transform.k, r); ctx.moveTo(-r, -r + rr2); ctx.quadraticCurveTo(-r, -r, -r + rr2, -r); ctx.lineTo(r - rr2, -r); ctx.quadraticCurveTo(r, -r, r, -r + rr2); ctx.lineTo(r, r - rr2); ctx.quadraticCurveTo(r, r, r - rr2, r); ctx.lineTo(-r + rr2, r); ctx.quadraticCurveTo(-r, r, -r, r - rr2); ctx.closePath(); }
    else if (n.kind === 'interface' || n.kind === 'type') { for (let i = 0; i < 6; i++) { const a = (Math.PI / 3) * i - Math.PI / 6; ctx.lineTo(r * Math.cos(a), r * Math.sin(a)); } ctx.closePath(); }
    else { ctx.arc(0, 0, r * 0.7, 0, Math.PI * 2); }
    ctx.fillStyle = color + '22'; ctx.fill();
    ctx.strokeStyle = borderColor; ctx.lineWidth = bw / transform.k; ctx.stroke();
    ctx.shadowBlur = 0;
    if (transform.k > 0.3) {
...
let gl = null, glProgram = null, glCanvas = null;
...
export function initWebGL() {
  ...
  try {
    ...
    gl = glCanvas.getContext('webgl', { antialias: true, alpha: false }) || glCanvas.getContext('experimental-webgl', { antialias: true, alpha: false });
    if (!gl) return false;
    const vs = gl.createShader(gl.VERTEX_SHADER);
    gl.shaderSource(vs, 'attribute vec2 aPos;uniform vec2 uOrigin;uniform float uScale;uniform vec2 uRes;void main(){vec2 p=(aPos*uScale+uOrigin)/uRes*2.0-1.0;p.y=-p.y;gl_Position=vec4(p,0.0,1.0);gl_PointSize=6.0;}');
    gl.compileShader(vs);
