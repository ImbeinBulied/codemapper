import {
  nodes, edges, nodeMap, sim, simSettled, layoutMode,
  setSim, setSimSettled, ViewNode,
} from './state.js';
...
export function startForceSimulation() {
  ...
  const s = d3.forceSimulation(nodes)
    ...
    .force('collision', d3.forceCollide().radius((d: any) => NODE_SIZE[d.kind] || 20))
    .alphaDecay(0.05)
    .on('tick', () => { computeDirectoryClusters(); render(); })
    .on('end', () => { setSimSettled(true); });
  setSim(s);
}

export function stopSimulation() {
  if (sim) { sim.stop(); setSim(null); }
}
