import {
  ...
  focusNode, sim, simSettled, hiddenKinds, layoutMode,
  setHoveredNode, setHoveredEdge, setSelectedNode,
  setIsDragging, setIsPanning, setDragNode, setPanStart,
  setFocusNode, setSimSettled, setContextMenuNode,
  contextMenuNode, ViewNode, ViewEdge,
} from './state.js';
...
function hitTest(cx: number, cy: number): ViewNode | null {
  let best: ViewNode | null = null, bestDist = Infinity;
  for (const n of nodes) {
    ...
    if (dist < size * size && dist < bestDist) { best = n; bestDist = dist; }
  }
  return best;
}

function hitTestEdge(cx: number, cy: number): ViewEdge | null {
  let best: ViewEdge | null = null, bestDist = 150;
...
container.addEventListener('mousedown', (e: MouseEvent) => {
  ...
  if (hit) {
    setIsDragging(true); setDragNode(hit);
...
container.addEventListener('mousemove', (e: MouseEvent) => {
  ...
  if (isDragging && dragNode) {
    ...
    if (sim && simSettled) { sim.alphaTarget(0.15).restart(); setSimSettled(false); }
    render();
    return;
  }
  ...
  if (hit !== hoveredNode || edgeHit !== hoveredEdge) {
    ...
    if (edgeHit && !hit) {
      ...
      const edgesInfo = edgeLabelsArr.length ? edgeLabelsArr.slice(0, 5).map(l => '<div style="color:#8b949e;font-size:10px">→ ' + l + '</div>').join('') : '';
      tooltip.innerHTML = '<div class="tt-label">' + hit.label + '</div><div class="tt-path">' + hit.filePath + ':' + hit.line + '</div>' +
        (hit.description ? '<div class="tt-desc">' + hit.description + '</div>' : '') + edgesInfo;
      tooltip.style.left = (e.clientX + 12) + 'px';
      tooltip.style.top = (e.clientY + 12) + 'px';
      clampTooltip();
    } else {
...
container.addEventListener('mouseup', () => {
  if (isDragging && dragNode) {
    ...
    setIsDragging(false); setDragNode(null);
...
container.addEventListener('mouseleave', () => {
  setIsPanning(false);
  setHoveredNode(null); setHoveredEdge(null);
  tooltip.style.display = 'none';
  render();
});
...
container.addEventListener('contextmenu', (e: MouseEvent) => {
  ...
  contextMenu.innerHTML =
    '<button class="cm-item" onclick="window.copyPath(\'' + hit.filePath.replace(/'/g, "\\'") + '\')">Copy path</button>' +
    '<div class="cm-sep"></div>' +
    '<button class="cm-item" onclick="window.setFocusNode(' + nodes.indexOf(hit) + ')">Show dependents</button>' +
    '<button class="cm-item" onclick="window.focusKind(\'' + hit.kind + '\')">Show only ' + hit.kind + '</button>' +
    '<button class="cm-item" onclick="window.hideKind(\'' + hit.kind + '\')">Hide ' + hit.kind + '</button>';
...
container.addEventListener('touchstart', (e: TouchEvent) => {
  if (e.touches.length === 1) {
    ...
    const hit = hitTest(p.x, p.y);
    if (hit) { setIsDragging(true); setDragNode(hit); if (sim && !simSettled) sim.alphaTarget(0.3).restart(); }
    else { setIsPanning(true); touchPanStart = { x: t.clientX - transform.x, y: t.clientY - transform.y }; }
...
container.addEventListener('touchmove', (e: TouchEvent) => {
  if (e.touches.length === 1) {
    ...
    const cx2 = (t1.clientX + t2.clientX) / 2, cy2 = (t1.clientY + t2.clientY) / 2;
    if (lastTouchDist > 0) {
...
container.addEventListener('touchend', () => {
  ...
  setIsPanning(false); lastTouchDist = 0; touchPanStart = null;
}, { passive: true });

// Exposed for HTML onclick
(window as any).copyPath = (fp: string) => { navigator.clipboard.writeText(fp).catch(() => {}); contextMenu.classList.remove('show'); };
(window as any).setFocusNode = (idx: number) => { setFocusNode(nodes[idx] || null); contextMenu.classList.remove('show'); render(); };
(window as any).focusKind = (kind: string) => {
  for (const k of ['file', 'function', 'class', 'interface', 'type', 'module', 'call']) { hiddenKinds[k] = k !== kind; }
  updateFilterButtons(); contextMenu.classList.remove('show'); render();
};
