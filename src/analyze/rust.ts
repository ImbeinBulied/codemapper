...
export async function analyzeRust(dir: string, rootDir: string, config?: Config): Promise<CodeGraph> {
  ...
  for (const filePath of rsFiles) {
    ...
    nodes.push({
      id: nodeId, label: relPath.split('/').pop() || relPath,
      kind: 'file', filePath: relPath, line: 1, col: 1, description: relPath,
    });
    ...
    for (const match of content.matchAll(STRUCT_RE)) {
      localFuncs.add(match[1]);
      const lineNum = findLine(lines, match[1]);
      nodes.push({ id: `struct:${relPath}#${match[1]}`, label: match[1], kind: 'type', filePath: relPath, line: lineNum, col: 1, description: 'struct' });
    ...
    for (const match of content.matchAll(ENUM_RE)) {
      localFuncs.add(match[1]);
      const lineNum = findLine(lines, match[1]);
      nodes.push({ id: `enum:${relPath}#${match[1]}`, label: match[1], kind: 'type', filePath: relPath, line: lineNum, col: 1, description: 'enum' });
    ...
    for (const match of content.matchAll(TRAIT_RE)) {
      localFuncs.add(match[1]);
      const lineNum = findLine(lines, match[1]);
      nodes.push({ id: `trait:${relPath}#${match[1]}`, label: match[1], kind: 'interface', filePath: relPath, line: lineNum, col: 1, description: 'trait' });
    ...
    for (const match of content.matchAll(IMPL_RE)) {
      ...
      if (!nodes.some(n => n.id === `impl:${relPath}#${target}`)) {
        nodes.push({ id: `impl:${relPath}#${target}`, label: `impl ${target}`, kind: 'call', filePath: relPath, line: findLine(lines, 'impl'), col: 1 });
    ...
    for (const match of content.matchAll(TYPE_RE)) {
      localFuncs.add(match[1]);
      const lineNum = findLine(lines, match[1]);
      nodes.push({ id: `typealias:${relPath}#${match[1]}`, label: match[1], kind: 'type', filePath: relPath, line: lineNum, col: 1, description: 'type alias' });
