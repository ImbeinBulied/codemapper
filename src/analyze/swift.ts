...
export async function analyzeSwift(dir: string, rootDir: string, config?: Config): Promise<CodeGraph> {
  ...
  for (const filePath of swiftFiles) {
    ...
    for (const m of content.matchAll(STRUCT_RE)) {
      localFuncs.add(m[1]);
      nodes.push({ id: `struct:${relPath}#${m[1]}`, label: m[1], kind: 'type', filePath: relPath, line: findLine(lines, m[1]), col: 1, description: 'struct' });
    ...
    for (const m of content.matchAll(PROTOCOL_RE)) {
      localFuncs.add(m[1]);
      nodes.push({ id: `protocol:${relPath}#${m[1]}`, label: m[1], kind: 'interface', filePath: relPath, line: findLine(lines, m[1]), col: 1, description: 'protocol' });
      edges.push({ source: nodeId, target: `protocol:${relPath}#${m[1]}`, kind: 'contains' });
    }
    for (const m of content.matchAll(EXTENSION_RE)) {
      nodes.push({ id: `ext:${relPath}#${m[1]}`, label: `ext ${m[1]}`, kind: 'call', filePath: relPath, line: findLine(lines, m[1]), col: 1, description: 'extension' });
    ...
    for (const m of content.matchAll(TYPEALIAS_RE)) {
      localFuncs.add(m[1]);
      nodes.push({ id: `typealias:${relPath}#${m[1]}`, label: m[1], kind: 'type', filePath: relPath, line: findLine(lines, m[1]), col: 1, description: 'typealias' });
      edges.push({ source: nodeId, target: `typealias:${relPath}#${m[1]}`, kind: 'contains' });
    }

    for (const m of content.matchAll(EXTENDS_RE)) {
      if (m[1].includes('=')) continue;
      for (const base of m[1].split(',').map(s => s.trim().replace(/<.*>/, '')).filter(Boolean)) {
