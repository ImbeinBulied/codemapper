...
const CALL_RE = /(\w+)\s*\(/g;
const SKIP_CALLS = /^(if|else|for|range|switch|case|default|select|go|defer|return|break|continue|fallthrough|chan|const|func|import|package|var|type|struct|interface|map|make|new|len|cap|append|copy|delete|close|panic|recover|print|println|error|true|false|nil|iota|any|comparable|int|int8|int16|int32|int64|uint|uint8|uint16|uint32|uint64|uintptr|float32|float64|complex64|complex128|bool|byte|rune|string|fmt|log|os|io|strings|strconv|errors|context|sync|math|sort|time|net|http|json|testing)$/;
...
export async function analyzeGo(dir: string, rootDir: string, config?: Config): Promise<CodeGraph> {
  ...
  for (const filePath of goFiles) {
    ...
    for (const m of content.matchAll(IMPORT_RE)) {
      ...
      if (m[1]) {
        for (const im of block.matchAll(IMPORT_LINE_RE)) {
          edges.push({ source: nodeId, target: `module:${im[1]}`, kind: 'imports', label: im[1].split('/').pop() || im[1] });
        }
      } else {
    ...
    for (const m of content.matchAll(FUNC_RE)) {
      localFuncs.add(m[1]);
      nodes.push({ id: `func:${relPath}#${m[1]}`, label: m[1], kind: 'function', filePath: relPath, line: findLine(lines, 'func ' + m[1]), col: 1 });
