...
export function toSVG(result: AnalysisResult): string {
  ...
  const shape: Record<string, string> = {
    file: 'circle', function: 'diamond', class: 'rect',
    interface: 'hexagon', type: 'hexagon', module: 'circle', call: 'circle',
  };
  ...
  for (const n of layoutNodes) {
    ...
    if (n.kind === 'file') {
      tag = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${color}" opacity="0.8"/>`;
    } else if (n.kind === 'function') {
      const points = `${cx},${cy - r} ${cx + r},${cy} ${cx},${cy + r} ${cx - r},${cy}`;
      tag = `<polygon points="${points}" fill="${color}" opacity="0.8"/>`;
    } else if (n.kind === 'class') {
      tag = `<rect x="${cx - r}" y="${cy - r}" width="${r * 2}" height="${r * 2}" rx="2" fill="${color}" opacity="0.8"/>`;
      ...
      tag = `<polygon points="${pts.join(' ')}" fill="${color}" opacity="0.8"/>`;
    } else {
      tag = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${color}" opacity="0.8"/>`;
    }
