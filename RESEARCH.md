  ...
  return Math.max(
    ...
    Math.min(
      ...
      171 -
        ...
        0.23 * complexity -
        16.2 * Math.log(Math.max(1, loc * (1 - commentRatio))),
    ),
  );
}
```
...
>
  ...
  <div role="list" aria-label="Graph nodes">
    ...
    <div role="listitem" tabindex="0" aria-label="Function fetchData in src/api.ts, line 42, imported by 3 files"></div>
  </div>
</div>
```
